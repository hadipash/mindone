...
from fastvideo.src.configs.sample import SamplingParam
from fastvideo.src.entrypoints.args import FastVideoArgs
from fastvideo.src.pipelines.batch_info import ForwardBatch
from fastvideo.src.utils import align_to, shallow_asdict
from fastvideo.src.worker import Executor
...
class VideoGenerator:
    ...
    @classmethod
    def from_pretrained(cls, model_path: str, mindspore_dtype: Optional[str] = None, **kwargs) -> "VideoGenerator":
        """
    ...
    def generate_video(
        ...
        **kwargs,
        ...
        num_npus = fastvideo_args.num_npus
        ...
        if orig_latent_num_frames % fastvideo_args.num_npus != 0:
            ...
            if sampling_param.num_frames_round_down:
                ...
                new_latent_num_frames = max(1, (orig_latent_num_frames // num_npus)) * num_npus
            else:
                new_latent_num_frames = math.ceil(orig_latent_num_frames / num_npus) * num_npus
            ...
            if use_temporal_scaling_frames:
                ...
                # Find the least common multiple of 3 and num_npus
                divisor = math.lcm(3, num_npus)
            ...
            logger.info(
                "Adjusting number of frames from {} to {} based on number of GPUs ({})",
                sampling_param.num_frames,
                new_num_frames,
                fastvideo_args.num_npus,
            )
     ...
     embedded_guidance_scale: {fastvideo_args.pipeline_config.embedded_cfg_scale}
        ...
        if batch.save_video:
            ...
            if output_path:
                ...
                logger.info("Saved video to {}", video_path)
            else:
