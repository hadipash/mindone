...
from fastvideo.src.configs.pipelines.hunyuan import HunyuanConfig
from fastvideo.src.utils import maybe_download_model_index, verify_model_config_and_directory
...
def get_pipeline_config_cls_from_name(pipeline_name_or_path: str) -> type[PipelineConfig]:
    ...
    if pipeline_config_cls is None:
        ...
        if pipeline_config_cls is not None:
            logger.warning(
                "No match found for pipeline {}, using fallback config {}.", pipeline_name_or_path, pipeline_config_cls
            )
