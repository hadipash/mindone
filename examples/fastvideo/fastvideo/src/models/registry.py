...
from typing import NoReturn, Optional, TypeVar, Union, cast
...
_SUBPROCESS_COMMAND = [sys.executable, "-m", "fastvideo.v1.models.dits.registry"]
...
class _ModelInfo:
    ...
    @staticmethod
    def from_model_cls(model: type[nn.Cell]) -> "_ModelInfo":
        return _ModelInfo(
...
class _BaseRegisteredModel(ABC):
    ...
    @abstractmethod
    def load_model_cls(self) -> type[nn.Cell]:
        raise NotImplementedError
...
class _RegisteredModel(_BaseRegisteredModel):
    ...
    @staticmethod
    def from_model_cls(model_cls: type[nn.Cell]):
...
class _LazyRegisteredModel(_BaseRegisteredModel):
    ...
    def load_model_cls(self) -> type[nn.Cell]:
        ...
        return cast(type[nn.Cell], getattr(mod, self.class_name))
...
) -> Optional[type[nn.Cell]]:
    from fastvideo.src.platforms import current_platform
...
class _ModelRegistry:
    ...
    def register_model(
        self,
        model_arch: str,
        model_cls: Union[type[nn.Cell], str],
    ) -> None:
        """
        ...
        - A :class:`torch.nn.Cell` class directly referencing the model.
    ...
    def _try_load_model_cls(self, model_arch: str) -> Optional[type[nn.Cell]]:
    ...
    def resolve_model_cls(self, architectures: Union[str, list[str]]) -> tuple[type[nn.Cell], str]:
        architectures = self._normalize_archs(architectures)
