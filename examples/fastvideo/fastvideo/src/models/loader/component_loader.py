...
from fastvideo.src.configs.models import EncoderConfig
from fastvideo.src.entrypoints.args import FastVideoArgs
from fastvideo.src.models.hf_transformer_utils import get_diffusers_config
from fastvideo.src.models.loader.fsdp_load import maybe_load_fsdp_model
from fastvideo.src.models.loader.weight_utils import (
    filter_duplicate_safetensors_files,
    filter_files_not_needed_for_inference,
    pt_weights_iterator,
    safetensors_weights_iterator,
)
from fastvideo.src.models.registry import ModelRegistry
from fastvideo.src.utils import PRECISION_TO_TYPE
...
class ComponentLoader(ABC):
    ...
    def for_module_type(cls, module_type: str, transformers_or_diffusers: str) -> "ComponentLoader":
        ...
        logger.warning("No specific loader found for module type: %s. Using generic loader.", module_type)
...
class TextEncoderLoader(ComponentLoader):
    ...
    def _get_weights_iterator(self, source: "Source", to_cpu: bool) -> Generator[tuple[str, Tensor], None, None]:
    ...
    def load(self, model_path: str, fastvideo_args: FastVideoArgs):
        ...
        return self.load_model(model_path, encoder_config, fastvideo_args, encoder_precision)
    ...
    def load_model(
        ...
        use_cpu_offload = (
            fastvideo_args.text_encoder_offload and len(getattr(model_config, "_fsdp_shard_conditions", [])) > 0
        )
        ...
        if weights_not_loaded:
            raise ValueError("Following weights were not initialized from " f"checkpoint: {weights_not_loaded}")
...
class TransformerLoader(ComponentLoader):
    ...
    def load(self, model_path: str, fastvideo_args: FastVideoArgs):
        ...
        logger.info("Loading model from %s safetensors files in %s", len(safetensors_list), model_path)
...
class PipelineComponentLoader:
    ...
    @staticmethod
    def load_module(
        module_name: str, component_model_path: str, transformers_or_diffusers: str, fastvideo_args: FastVideoArgs
    ):
        """
        ...
        Returns:
            The loaded module
        """
        logger.info("Loading %s using %s from %s", module_name, transformers_or_diffusers, component_model_path)
