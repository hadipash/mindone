...
def pt_weights_iterator(
    ...
    raise NotImplementedError("PyTorch weights are not supported yet.")


def default_weight_loader(param: Tensor, loaded_weight: Tensor) -> None:
    ...
    try:
        if param.numel() == 1 and loaded_weight.numel() == 1:
            ...
            assert param.size() == loaded_weight.size(), (
                f"Attempted to load weight ({loaded_weight.size()}) " f"into parameter ({param.size()})"
            )
        ...
        if name.endswith(scale_name):
            if any(mo_scale_name in name for mo_scale_name in modelopt_scale_names):
                remapped_name = name.replace(
                    f".self_attn.{scale_name[1]}_proj{scale_name}", f".self_attn.attn{scale_name}"
                )
            else:
