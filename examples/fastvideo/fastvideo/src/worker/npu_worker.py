...
import psutil
from fastvideo.src.distributed import cleanup_dist_env_and_memory, maybe_init_distributed_environment_and_model_parallel
from fastvideo.src.entrypoints.args import FastVideoArgs
from fastvideo.src.pipelines import ForwardBatch, build_pipeline
from fastvideo.src.utils import get_exception_traceback, kill_itself_when_parent_died
...
class Worker:
    def __init__(self, fastvideo_args: FastVideoArgs, local_rank: int, rank: int, pipe: Connection, master_port: int):
    ...
    def init_device(self) -> None:
        ...
        os.environ["WORLD_SIZE"] = str(self.fastvideo_args.num_npus)
    ...
    def set_lora_adapter(self, lora_nickname: str, lora_path: Optional[str] = None) -> None:
        self.pipeline.set_lora_adapter(lora_nickname, lora_path)

    def shutdown(self) -> dict[str, Any]:
        """Gracefully shut down the worker process"""
        logger.info("Worker {} shutting down...", self.rank, local_main_process_only=False)
        # Clean up resources
        ...
        logger.info("Worker {} shutdown complete", self.rank, local_main_process_only=False)
        return {"status": "shutdown_complete"}

    def event_loop(self) -> None:
        """Event loop for the worker."""
        logger.info("Worker {} starting event loop...", self.rank, local_main_process_only=False)
        while True:
            try:
                ...
                if method_name == "execute_forward":
                    ...
                    output_batch = self.execute_forward(forward_batch, fastvideo_args)
                    ...
                    logger.info("Worker {} set LoRA adapter {} with path {}", self.rank, lora_nickname, lora_path)
                ...
                logger.error("Worker {} in loop received KeyboardInterrupt, aborting forward pass", self.rank)
                try:
                    self.pipe.send({"error": "Operation aborted by KeyboardInterrupt"})
                    logger.info("Worker {} sent error response after interrupt", self.rank)
                except Exception as e:
                    logger.error("Worker {} failed to send error response: {}", self.rank, str(e))
                continue


def run_worker_process(fastvideo_args: FastVideoArgs, local_rank: int, rank: int, pipe: Connection, master_port: int):
    ...
    logger.info("Worker {} initializing...", rank, local_main_process_only=False)

    try:
        worker = Worker(fastvideo_args, local_rank, rank, pipe, master_port)
        logger.info("Worker {} sending ready", rank)
        pipe.send(
            {
                "status": "ready",
        ...
        logger.error("Worker {} hit an exception: {}", rank, traceback)
