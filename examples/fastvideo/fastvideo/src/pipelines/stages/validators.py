...
from mindspore import Generator, Tensor, mint
...
class StageValidators:
    ...
    def is_tensor(value: Any) -> bool:
        ...
        if not isinstance(value, Tensor):
            return False
        return not mint.isnan(value).any().item()
    ...
    def generator_or_list_generators(value: Any) -> bool:
        ...
        if isinstance(value, Generator):
            return True
        if isinstance(value, list):
            return all(isinstance(item, Generator) for item in value)
        return False
...
class VerificationResult:
    ...
    def _create_validation_failure(self, validator: Callable, value: Any) -> ValidationFailure:
        ...
        if validator_name == "is_tensor":
            expected = "Tensor without NaN values"
            if isinstance(value, Tensor) and mint.isnan(value).any().item():
                error_msg = f"tensor contains {mint.isnan(value).sum().item()} NaN values"
            ...
            if isinstance(value, Tensor):
                if mint.isnan(value).any().item():
                    error_msg = (
                        f"tensor has {value.dim()} dimensions but contains {mint.isnan(value).sum().item()} NaN values"
                    )
                else:
            ...
            if isinstance(value, list) and len(value) > 0:
                nan_count = 0
                for item in value:
                    if isinstance(item, Tensor) and mint.isnan(item).any().item():
                        nan_count += mint.isnan(item).sum().item()
                if nan_count > 0:
