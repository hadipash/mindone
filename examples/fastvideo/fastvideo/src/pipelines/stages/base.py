...
from fastvideo.src.pipelines.stages.validators import VerificationResult
...
class PipelineStage(ABC):
    ...
    def __call__(self, batch: ForwardBatch, fastvideo_args: FastVideoArgs) -> ForwardBatch:
        """
        ...
        if False:
            ...
            try:
                ...
                self._logger.info("[%s] Execution completed in %s ms", stage_name, execution_time * 1000)
            except Exception as e:
                execution_time = time.perf_counter() - start_time
                self._logger.error("[%s] Error during execution after %s ms: %s", stage_name, execution_time * 1000, e)
                self._logger.error("[%s] Traceback: %s", stage_name, traceback.format_exc())
                raise
        else:
    ...
    def backward(self, batch: ForwardBatch, fastvideo_args: FastVideoArgs) -> ForwardBatch:
        raise NotImplementedError
