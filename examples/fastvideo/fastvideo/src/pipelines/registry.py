...
class _PipelineRegistry:
    ...
    pipelines: dict[str, Optional[type[ComposedPipelineBase]]] = field(default_factory=dict)

    def get_supported_archs(self) -> Set[str]:
        return self.pipelines.keys()

    def _try_load_pipeline_cls(self, pipeline_arch: str) -> Union[type[ComposedPipelineBase], None]:
...
def import_pipeline_classes():
    ...
    package_name = "fastvideo.src.pipelines"
    ...
    for _, name, ispkg in pkgutil.iter_modules(package.__path__, package_name + "."):
        if ispkg:
            ...
            for _, name, ispkg in pkgutil.iter_modules(sub_package.__path__, sub_package_name + "."):
                try:
                    module = importlib.import_module(name)
                except Exception as e:
                    logger.warning("Ignore import error when loading {}. {}", name, e)
                    continue
                if hasattr(module, "EntryClass"):
                    entry = module.EntryClass
                    if isinstance(entry, list):  # To support multiple pipeline classes in one module
                        for tmp in entry:
                            assert (
