...
def Flux(
    ...
    dtype: mstype.Type = mstype.bfloat16,
    **kwargs,
) -> MMDiTModel:
    config = MMDiTConfig(
    ...
    if from_pretrained:
        ...
        _logger.info("net param not load: ", m, len(m))
        _logger.info("ckpt param not load: ", u, len(u))
        _logger.info(f"Loaded ckpt {ckpt_path} into STDiT3.")
    return model
