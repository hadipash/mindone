...
class DBCacheContext:
    ...
    Fn_compute_blocks_ids: list[int] = field(default_factory=list)
    Bn_compute_blocks_ids: list[int] = field(default_factory=list)
    ...
    residual_diff_threshold: Union[Tensor, float] = 0.05
    ...
    alter_residual_diff_threshold: Optional[Union[Tensor, float]] = 1.0
    ...
    taylorseer_kwargs: dict[str, Any] = field(default_factory=dict)
    ...
    residual_diffs: DefaultDict[str, float] = field(default_factory=lambda: defaultdict(float))
    continuous_cached_steps: int = 0
    cfg_cached_steps: list[int] = field(default_factory=list)
    cfg_residual_diffs: DefaultDict[str, float] = field(default_factory=lambda: defaultdict(float))
    ...
    def __post_init__(self):
        ...
        if self.do_separate_cfg:
            assert self.enable_alter_cache is False, (
                "enable_alter_cache must set as False if " "do_separate_cfg is enabled."
            )
            if self.cfg_diff_compute_separate:
                assert self.cfg_compute_first is False, (
                    "cfg_compute_first must set as False if " "cfg_diff_compute_separate is enabled."
                )
        ...
        if "max_warmup_steps" not in self.taylorseer_kwargs:
            ...
            self.taylorseer_kwargs["max_warmup_steps"] = self.max_warmup_steps if self.max_warmup_steps > 0 else 1

        # Only set n_derivatives as 2 or 3, which is enough for most cases.
        if "n_derivatives" not in self.taylorseer_kwargs:
            self.taylorseer_kwargs["n_derivatives"] = max(2, min(3, self.taylorseer_kwargs["max_warmup_steps"]))
        ...
        if self.enable_encoder_taylorseer:
            ...
            if self.do_separate_cfg:
                self.cfg_encoder_taylorseer = TaylorSeer(**self.taylorseer_kwargs)
    ...
    def get_residual_diff_threshold(self):
        ...
        if isinstance(residual_diff_threshold, Tensor):
    ...
    def mark_step_begin(self):
        ...
        if self.get_current_transformer_step() == 0:
            ...
            if self.enable_taylorseer or self.enable_encoder_taylorseer:
                ...
                cfg_taylorseer, cfg_encoder_taylorseer = self.get_cfg_taylorseers()
...
def Fn_compute_blocks():
    cache_context = get_current_cache_context()
    assert cache_context is not None, "cache_context must be set before"
    assert cache_context.Fn_compute_blocks >= 1, "Fn_compute_blocks must be >= 1"
    if cache_context.max_Fn_compute_blocks > 0:
        ...
        assert cache_context.Fn_compute_blocks <= cache_context.max_Fn_compute_blocks, (
...
def Fn_compute_blocks_ids():
    cache_context = get_current_cache_context()
    assert cache_context is not None, "cache_context must be set before"
    assert len(cache_context.Fn_compute_blocks_ids) <= cache_context.Fn_compute_blocks, (
...
def Bn_compute_blocks():
    cache_context = get_current_cache_context()
    assert cache_context is not None, "cache_context must be set before"
    assert cache_context.Bn_compute_blocks >= 0, "Bn_compute_blocks must be >= 0"
    if cache_context.max_Bn_compute_blocks > 0:
        ...
        assert cache_context.Bn_compute_blocks <= cache_context.max_Bn_compute_blocks, (
...
def Bn_compute_blocks_ids():
    cache_context = get_current_cache_context()
    assert cache_context is not None, "cache_context must be set before"
    assert len(cache_context.Bn_compute_blocks_ids) <= cache_context.Bn_compute_blocks, (
...
def collect_cache_kwargs(default_attrs: dict, **kwargs):
    ...
    cache_attrs = fields(DBCacheContext)
    cache_attrs = [attr for attr in cache_attrs if hasattr(DBCacheContext, attr.name)]
    cache_kwargs = {attr.name: kwargs.pop(attr.name, getattr(DBCacheContext, attr.name)) for attr in cache_attrs}

    def _safe_set_sequence_field(field_name: str, default_value: Any = None):
        if field_name not in cache_kwargs:
            cache_kwargs[field_name] = kwargs.pop(field_name, default_value)
...
def are_two_tensors_similar(
    t1: Tensor,  # prev residual R(t-1,n) = H(t-1,n) - H(t-1,0)
    t2: Tensor,  # curr residual R(t  ,n) = H(t  ,n) - H(t  ,0)
    *,
    threshold: float,
...
def _debugging_set_buffer(prefix):
    logger.debug(
        f"set {prefix}, " f"transformer step: {get_current_transformer_step()}, " f"executed step: {get_current_step()}"
    )


def _debugging_get_buffer(prefix):
    logger.debug(
        f"get {prefix}, " f"transformer step: {get_current_transformer_step()}, " f"executed step: {get_current_step()}"
    )
...
def set_Fn_buffer(buffer: Tensor, prefix: str = "Fn"):
...
def set_Fn_encoder_buffer(buffer: Tensor, prefix: str = "Fn"):
    if is_separate_cfg_step():
...
def set_Bn_buffer(buffer: Tensor, prefix: str = "Bn"):
    ...
    if is_taylorseer_enabled():
        ...
        if taylorseer is not None:
            ...
            logger.debug(
                "TaylorSeer is enabled but not set in the cache context. " "Falling back to default buffer retrieval."
            )
            if is_separate_cfg_step():
                _debugging_set_buffer(f"{prefix}_buffer_cfg")
                set_buffer(f"{prefix}_buffer_cfg", buffer)
...
def set_Bn_encoder_buffer(buffer: Tensor, prefix: str = "Bn"):
...
def apply_hidden_states_residual(
    hidden_states: Tensor, encoder_hidden_states: Tensor = None, prefix: str = "Bn", encoder_prefix: str = "Bn_encoder"
):
    ...
    if encoder_hidden_states is not None:
        ...
        assert encoder_hidden_states_prev is not None, f"{prefix}_encoder_buffer must be set before"
...
def get_can_use_cache(
    states_tensor: Tensor,  # hidden_states or residual
    parallelized: bool = False,
    ...
    if max_cached_steps >= 0 and (len(cached_steps) >= max_cached_steps):
        logger.debug(f"{prefix}, max_cached_steps reached: {max_cached_steps}, " "can not use cache.")
        return False
    ...
    if max_continuous_cached_steps >= 0 and (continuous_cached_steps >= max_continuous_cached_steps):
        logger.debug(
            f"{prefix}, max_continuous_cached_steps " f"reached: {max_continuous_cached_steps}, " "can not use cache."
        )
    ...
    if not is_alter_cache_enabled():
        ...
        can_use_cache = prev_states_tensor is not None and are_two_tensors_similar(
            prev_states_tensor, states_tensor, threshold=threshold, parallelized=parallelized, prefix=prefix
        )
    else:
