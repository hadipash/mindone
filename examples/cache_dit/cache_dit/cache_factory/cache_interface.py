...
from cache_dit.cache_factory.cache_adapters import BlockAdapter, UnifiedCacheAdapter
...
def enable_cache(
    ...
    **other_cache_kwargs,
    ...
    cache_context_kwargs["max_continuous_cached_steps"] = max_continuous_cached_steps
    ...
    cache_context_kwargs["cfg_diff_compute_separate"] = cfg_diff_compute_separate
    cache_context_kwargs["enable_taylorseer"] = enable_taylorseer
    cache_context_kwargs["enable_encoder_taylorseer"] = enable_encoder_taylorseer
    cache_context_kwargs["taylorseer_cache_type"] = taylorseer_cache_type
    if "taylorseer_kwargs" in cache_context_kwargs:
        cache_context_kwargs["taylorseer_kwargs"]["n_derivatives"] = taylorseer_order
    else:
        cache_context_kwargs["taylorseer_kwargs"] = {"n_derivatives": taylorseer_order}

    if isinstance(pipe_or_adapter, BlockAdapter):
        return UnifiedCacheAdapter.apply(
            pipe=None, block_adapter=pipe_or_adapter, forward_pattern=forward_pattern, **cache_context_kwargs
        )
    elif isinstance(pipe_or_adapter, DiffusionPipeline):
        return UnifiedCacheAdapter.apply(
            pipe=pipe_or_adapter, block_adapter=None, forward_pattern=forward_pattern, **cache_context_kwargs
        )
    else:
        raise ValueError("Please pass DiffusionPipeline or BlockAdapter" "for the 1's position param: pipe_or_adapter")
