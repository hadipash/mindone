...
from mindspore import Tensor, dtype, mint, nn

from mindone.diffusers import FluxTransformer2DModel
from mindone.diffusers.models.transformers.transformer_flux import FluxSingleTransformerBlock, Transformer2DModelOutput

# FIXME: USE_PEFT_BACKEND
# from mindone.diffusers.utils import USE_PEFT_BACKEND, scale_lora_layers, unscale_lora_layers
...
def __patch_single_forward__(
    ...
    temb: Tensor,
    image_rotary_emb: Optional[tuple[Tensor]] = None,
...
def __patch_transformer_forward__(
    ...
    if txt_ids.ndim == 3:
        logger.warning(
            "Passing `txt_ids` 3d Tensor is deprecated. Please remove the batch dimension and pass it as a 2d Tensor"
        )
        txt_ids = txt_ids[0]
    if img_ids.ndim == 3:
        logger.warning(
            "Passing `img_ids` 3d Tensor is deprecated. Please remove the batch dimension and pass it as a 2d Tensor"
        )
        img_ids = img_ids[0]
...
def maybe_patch_flux_transformer(
    transformer: FluxTransformer2DModel, blocks: nn.CellList = None
