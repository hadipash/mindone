...
from cache_dit.cache_factory.cache_blocks import DBCachedTransformerBlocks, cache_context
...
class BlockAdapter:
    ...
    transformer: nn.Cell = None
    blocks: nn.CellList = None
    # transformer_blocks, blocks, etc.
    blocks_name: str = None
    dummy_blocks_names: list[str] = field(default_factory=list)
    ...
    allow_suffixes: list[str] = field(default_factory=lambda: ["TransformerBlock"])
    check_suffixes: bool = False
    blocks_policy: str = field(default="max", metadata={"allowed_values": ["max", "min"]})
    ...
    def maybe_apply_patch(self):
        ...
        if self.transformer.__class__.__name__.startswith("Flux"):
            self.transformer = maybe_patch_flux_transformer(self.transformer, blocks=self.blocks)

    @staticmethod
    def auto_block_adapter(adapter: "BlockAdapter", forward_pattern: Optional[ForwardPattern] = None) -> "BlockAdapter":
        assert adapter.auto, (
            "Please manually set `auto` to True, or, manually " "set all the transformer blocks configuration."
        )
        ...
        return BlockAdapter(pipe=pipe, transformer=transformer, blocks=blocks, blocks_name=blocks_name)
    ...
    def check_block_adapter(adapter: "BlockAdapter") -> bool:
        if (
            ...
            and isinstance(adapter.blocks, nn.CellList)
        ):
            return True
        ...
        **kwargs,
        ...
        for blocks_name in blocks_names:
            if blocks := getattr(transformer, blocks_name, None):
                if isinstance(blocks, nn.CellList):
                    block = blocks[0]
                    block_cls_name = block.__class__.__name__
                    # Check suffixes
                    if isinstance(block, nn.Cell) and (
                        any((block_cls_name.endswith(allow_suffix) for allow_suffix in allow_suffixes))
                        or (not check_suffixes)
                    ):
                        # May check forward pattern
                        if forward_pattern is not None:
                            if BlockAdapter.match_blocks_pattern(blocks, forward_pattern, logging=False):
    ...
    @staticmethod
    def match_block_pattern(block: nn.Cell, forward_pattern: ForwardPattern) -> bool:
        assert (
        ...
        num_outputs = str(inspect.signature(block.forward).return_annotation).count("Tensor")
    ...
    def match_blocks_pattern(
        transformer_blocks: nn.CellList, forward_pattern: ForwardPattern, logging: bool = True
    ) -> bool:
        assert (
            forward_pattern.Supported and forward_pattern in ForwardPattern.supported_patterns()
        ), f"Pattern {forward_pattern} is not support now!"

        assert isinstance(transformer_blocks, nn.CellList)

        pattern_matched_states = []
        for block in transformer_blocks:
            pattern_matched_states.append(BlockAdapter.match_block_pattern(block, forward_pattern))
...
class UnifiedCacheAdapter:
    ...
    def get_params(cls, pipe: DiffusionPipeline) -> UnifiedCacheParams:
        ...
        if pipe_cls_name.startswith("Flux"):
            from mindone.diffusers import FluxTransformer2DModel
            ...
            from mindone.diffusers import WanTransformer3DModel, WanVACETransformer3DModel

            assert isinstance(pipe.transformer, (WanTransformer3DModel, WanVACETransformer3DModel))
            return UnifiedCacheParams(
                block_adapter=BlockAdapter(
                    pipe=pipe,
                    transformer=pipe.transformer,
    ...
    def apply(
        ...
        **cache_context_kwargs,
        ...
        assert pipe is not None or block_adapter is not None, "pipe or block_adapter can not both None!"
        ...
        if pipe is not None:
            if cls.is_supported(pipe):
                ...
                return cls.cachify(params.block_adapter, forward_pattern=params.forward_pattern, **cache_context_kwargs)
            else:
                raise ValueError(
            ...
            return cls.cachify(block_adapter, forward_pattern=forward_pattern, **cache_context_kwargs)
    ...
    def cachify(
        ...
        **cache_context_kwargs,
        ...
        if block_adapter.auto:
            block_adapter = BlockAdapter.auto_block_adapter(block_adapter, forward_pattern)
        ...
        if BlockAdapter.check_block_adapter(block_adapter):
            ...
            cls.patch_params(block_adapter, forward_pattern=forward_pattern, **cache_context_kwargs)
        return block_adapter.pipe

    @classmethod
    def patch_params(cls, block_adapter: BlockAdapter, forward_pattern: ForwardPattern = None, **cache_context_kwargs):
        ...
        block_adapter.pipe.__class__._cache_context_kwargs = cache_context_kwargs

    @classmethod
    def has_separate_cfg(cls, pipe_or_transformer: DiffusionPipeline | Any) -> bool:
    ...
    def check_context_kwargs(cls, pipe, **cache_context_kwargs):
        ...
        if cache_type := cache_context_kwargs.pop("cache_type", None):
            assert cache_type == CacheType.DBCache, "Custom cache setting only support for DBCache now!"

        return cache_context_kwargs

    @classmethod
    def create_context(cls, pipe: DiffusionPipeline, **cache_context_kwargs) -> DiffusionPipeline:
        if getattr(pipe, "_is_cached", False):
            return pipe

        # Check cache_context_kwargs
        cache_context_kwargs = cls.check_context_kwargs(pipe, **cache_context_kwargs)
        # Apply cache on pipeline: wrap cache context
        cache_kwargs, _ = cache_context.collect_cache_kwargs(default_attrs={}, **cache_context_kwargs)
        original_call = pipe.__class__.__call__

        @functools.wraps(original_call)
        def new_call(self, *args, **kwargs):
            with cache_context.cache_context(cache_context.create_cache_context(**cache_kwargs)):
    ...
    @classmethod
    def mock_blocks(
        cls, block_adapter: BlockAdapter, forward_pattern: ForwardPattern = ForwardPattern.Pattern_0
    ) -> nn.Cell:
        if getattr(block_adapter.transformer, "_is_cached", False):
            return block_adapter.transformer

        # Check block forward pattern matching
        assert BlockAdapter.match_blocks_pattern(block_adapter.blocks, forward_pattern=forward_pattern), (
            "No block forward pattern matched, " f"supported lists: {ForwardPattern.supported_patterns()}"
        )

        # Apply cache on transformer: mock cached transformer blocks
        cached_blocks = nn.CellList(
            [
                DBCachedTransformerBlocks(
                    block_adapter.blocks, transformer=block_adapter.transformer, forward_pattern=forward_pattern
                )
            ]
        )
        dummy_blocks = nn.CellList()
        ...
        def new_forward(self, *args, **kwargs):
            with ExitStack() as stack:
                stack.enter_context(unittest.mock.patch.object(self, block_adapter.blocks_name, cached_blocks))
        ...
        block_adapter.transformer.forward = new_forward.__get__(block_adapter.transformer)
