    ...
    try:
        ...
        for key in required_keys:
            if key not in config:
                raise ValueError(f"Configuration file missing required item: {key}")
        ...
        if isinstance(config["cache_type"], str):
            try:
                ...
                raise ValueError(
                    f"Invalid cache_type value: {config['cache_type']}, " f"valid values are: {valid_types}"
                )
        elif not isinstance(config["cache_type"], CacheType):
            raise ValueError(f"cache_type must be a string or CacheType enum, " f"got: {type(config['cache_type'])}")

        # Handle default value for taylorseer_kwargs
        if "taylorseer_kwargs" not in config and config.get("enable_taylorseer", False):
