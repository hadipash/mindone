...
class CacheStats:
    ...
    residual_diffs: dict[str, float] = field(default_factory=dict)
    cfg_cached_steps: list[int] = field(default_factory=list)
    cfg_residual_diffs: dict[str, float] = field(default_factory=dict)


def summary(
    pipe_or_transformer: Union[DiffusionPipeline, nn.Cell], details: bool = False, logging: bool = True
) -> CacheStats:
    ...
    if hasattr(transformer, "_cached_steps"):
        ...
        if residual_diffs and logging:
            ...
            print("| Cache Steps | Diffs P00 | Diffs P25 | Diffs P50 | Diffs P75 | Diffs P95 | Diffs Min | Diffs Max |")
            print("|-------------|-----------|-----------|-----------|-----------|-----------|-----------|-----------|")
            print(
            ...
            if details:
                ...
                pprint(f"Residual Diffs: {len(residual_diffs)}, {residual_diffs}", compact=True)

    if hasattr(transformer, "_cfg_cached_steps"):
        cfg_cached_steps: list[int] = transformer._cfg_cached_steps
        cfg_residual_diffs: dict[str, float] = dict(transformer._cfg_residual_diffs)
        ...
        if cfg_residual_diffs and logging:
            ...
            if details:
                ...
                pprint(f"CFG Cache Steps: {len(cfg_cached_steps)}, {cfg_cached_steps}")
                pprint(f"CFG Residual Diffs: {len(cfg_residual_diffs)}, {cfg_residual_diffs}", compact=True)

    return cache_stats


def strify(pipe_or_stats: Union[DiffusionPipeline, CacheStats, dict]):
